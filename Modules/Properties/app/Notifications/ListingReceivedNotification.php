    ...
    private function newAccount(object $notifiable, string $site, bool $en, string $title): MailMessage
    {
        ...
        return $en
            ...
            : (new MailMessage)
                ...
                ->line('يراجعها الفريق وينشرها خلال ٢٤ ساعة.')
                ->line('أنشأنا لك حسابًا لمتابعتها ولتصلك طلبات المشترين مباشرة. حدّد كلمة المرور لتفعيله:')
                ->action('حدّد كلمة السر', $link)
                ->line("تنتهي صلاحية الرابط بعد {$minutes} دقيقة — يمكنك طلب رابط جديد من «نسيت كلمة المرور».")
                ->line('إن لم تكن أنت من عرض هذه الوحدة، تجاهل الرسالة وتواصل معنا.')
    ...
    private function existingAccount(string $site, bool $en, string $title): MailMessage
    {
        ...
        return $en
            ...
            : (new MailMessage)
                ...
                ->greeting('أهلًا،')
                ->line("عُرضت وحدة جديدة على حسابك: {$title}.")
                ->line('يراجعها الفريق وينشرها خلال ٢٤ ساعة.')
                ->action('افتح «وحداتي»', $account)
                ->line('إن لم تكن أنت من فعل ذلك، ادخل حسابك واحذفها — أو تواصل معنا.')
                ->salutation("تحياتنا، {$site}");
    }
}
